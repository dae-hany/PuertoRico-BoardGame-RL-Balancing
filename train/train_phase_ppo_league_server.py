...
LEAGUE_PROB_SELF = 0.5
LEAGUE_PROB_PAST = 0.3
LEAGUE_PROB_BOTS = 0.2
...
def rollout_worker(rank, conn, shared_bufs, obs_dim, action_dim, opponent_pool, potential_type="base"):
    ...
    builder_bot = BuilderBot(action_dim=action_dim).eval()
    shipper_bot = ShipperBot(action_dim=action_dim).eval()
    random_bot = RandomBot(action_dim=action_dim).eval()
    league_bots = [builder_bot, shipper_bot, random_bot]
        ...
        if cmd == CMD_SET_WEIGHTS:
            ...
            if not env_initialized:
                ...
                if not opponent_pool:
                    opp_weights = None
                    selected_bot = random.choice(league_bots) if random.random() < LEAGUE_PROB_BOTS else None
                else:
                    rand_v = random.random()
                    if rand_v < LEAGUE_PROB_SELF:
                        opp_weights = None
                        selected_bot = None
                    elif rand_v < LEAGUE_PROB_SELF + LEAGUE_PROB_PAST:
                        opp_weights = random.choice(opponent_pool)
                        local_opponent.load_state_dict(opp_weights)
                        selected_bot = None
                    else:
                        opp_weights = None
                        selected_bot = random.choice(league_bots)
            ...
            while True:
                ...
                if is_learner:
                    ...
                    if selected_bot is not None:
                        agent_to_use = selected_bot
                    elif opp_weights is not None:
                        agent_to_use = local_opponent
                    else:
                        agent_to_use = local_agent
                    with torch.no_grad():
