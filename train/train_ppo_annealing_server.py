...
def rollout_worker(rank, conn, shared_bufs, obs_dim, action_dim, opponent_pool,
    ...
    while True:
        ...
        if cmd == CMD_SET_WEIGHTS:
            ...
            frac = data if data is not None else 1.0
            if hasattr(env, 'set_shaping_weight'):
                env.set_shaping_weight(frac)
...
def train(exp_name: str = "", w_ship: float = 1.0, w_bldg: float = 1.0, w_doub: float = 1.0):
    ...
    base_name = "AnnealingPPO_PR_Server"
    ...
    try:
        for update in range(1, total_updates + 1):
            ...
            writer.add_scalar("Hyperparameters/Shaping_Weight", frac, global_step)
            ...
            if total_games > 0:
                ...
                print(f" └─ WinRate: {win_rate:.2f} | Loss(P/V/E): {np.mean(losses_pg):.3f}/{np.mean(losses_v):.3f}/{np.mean(losses_ent):.3f} | Shaping W: {frac:.3f}\n")
